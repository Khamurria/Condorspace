...
class Planet {
    constructor(x, y, s, t, d) {
        this.x = x;
        this.y = y;
        this.size = s;
        this.type = t;
        this.depth = d;
        this.speed = window.starSpeed * (0.18 + d * 0.22);
        this.featureSeed = Math.floor(pseudoRandom() * 10000);
        this.rotationAngle = pseudoRandom() * Math.PI * 2;
        this.rotationSpeed = (pseudoRandom() - 0.5) * 0.0004 * (1 / (this.speed + 0.1));
        this.active = true;
    }
    ...
    update(deltaTime) {
        ...
        if (this.y - this.size > window.canvas.height) {
            this.active = false;
            
            if (window.currentPlanetTypes) {
                const index = window.currentPlanetTypes.indexOf(this.type);
                if (index > -1) {
                    window.currentPlanetTypes.splice(index, 1);
                }
            }
        }
    }
